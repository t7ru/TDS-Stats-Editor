...
async function uploadTower(towerData) {
    try {
        const username = document.getElementById('fandomUsername').value.trim();
        const towerName = document.getElementById('towerName').value.trim();
        const description = document.getElementById('towerDescription').value.trim();
        const imageUrl = document.getElementById('towerImageUrl').value.trim();
        ...
        if (jsonFileOption.checked) {
            ...
            jsonData = document.getElementById('towerJSONText').value.trim();
            try {
                // Parse to validate and then stringify to clean format
            ...
            jsonData = document.getElementById('towerJSONLink').value.trim();
        }
        ...
        // Make API request
        const response = await fetch('/api/towers/upload', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                ...
                author: username,
                imageUrl,
                towerData: jsonData ? JSON.parse(jsonData) : null,
                tag,
                verified: false // New towers start unverified
            }),
        });
        
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.error || 'Failed to upload tower');
        }
        
        const result = await response.json();
        
        // Show success message
        window.showAlert('Tower uploaded successfully!', 'success');
        
        // Refresh the tower list
        setTimeout(() => {
            document.getElementById('refresh-towers-btn').click();
        }, 1000);
